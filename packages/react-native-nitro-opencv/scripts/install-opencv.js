...
function filterRelease(release) {
    ...
    selectedAssets.push(...release.assets.filter((asset) => {
    ...
    if (selectedAssets.length > 0) {
        console.log("Downloading assets: ", selectedAssets.map(s => s.name), join(", "));
    }
    return true;
}
async function run() {
    ...
    try {
        const result = await downloadReleases(user, repo, tmpdir, filterRelease, filterAsset, false);
