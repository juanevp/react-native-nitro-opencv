...
import type { HoughModes } from "./constants-image-processing";
import type { CvMat } from "./mat.nitro";
export interface Feature extends HybridObject<{
    ios: "c++";
    android: "c++";
}> {
    /**
     ...
     * @param blockSize Neighborhood size
     */
    cornerMinEigenVal(src: CvMat, dst: CvMat, blockSize: number): void;
    /**
     ...
     */
    goodFeaturesToTrack(image: CvMat, corners: CvMat, maxCorners: number, qualityLevel: number, minDistance: number): void;
    /**
     ...
     */
    HoughCircles(image: CvMat, circles: CvMat, method: HoughModes, dp: number, minDist: number, param1?: number, param2?: number): void;
    /**
     ...
     * @param threshold Accumulator threshold parameter. Only those lines are returned that get enough votes ( >𝚝𝚑𝚛𝚎𝚜𝚑𝚘𝚕𝚍 ).
     */
    HoughLinesP(image: CvMat, lines: CvMat, rho: number, theta: number, threshold: number): void;
}
