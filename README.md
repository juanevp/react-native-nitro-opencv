...
- [`android/`](android): All your `android`-specific implementations.
  - [`build.gradle`](android/build.gradle): The gradle build file. This contains four important pieces:
    ...
    3. Adds all Nitrogen files (`apply from: '.../NitroOpencv+autolinking.gradle'`)
  ...
  - [`CMakeLists.txt`](android/CMakeLists.txt): The CMake build file to build C++ code. This contains four important pieces:
    1. Creates a library called `NitroOpencv` (same as in `nitro.json`)
    2. Adds all Nitrogen files (`include(.../NitroOpencv+autolinking.cmake)`)
  ...
  - [`src/main/java/com/margelo/nitro/nitroopencv/`](android/src/main/java/com/margelo/nitro/nitroopencv/): All Kotlin implementations.
    - [`NitroOpencvPackage.java`](android/src/main/java/com/margelo/nitro/nitroopencv/NitroOpencvPackage.java): The react-native package. You need this because the react-native CLI only adds libraries if they have a `*Package.java` file. In here, you can autolink all Kotlin HybridObjects.
...
- [`NitroOpencv.podspec`](NitroOpencv.podspec): The iOS podspec build file to build the iOS code. This contains three important pieces:
...
##Requirements for OpenCV Android build
Android Studio with some NDK installed
Python 3
Add to PATH C:\Users\<user>\AppData\Local\Android\Sdk\tools and C:\Users\<user>\AppData\Local\Android\Sdk\platform-tools
Create ANDROID_HOME environment variable. Optionally set ANDROID_NDK_HOME to the version of NDK to use.
